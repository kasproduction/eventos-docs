...
| **Mayo** | Webapp cimientos + core | **W.1 COMPLETA** (~18h reales — F0-F6 + F8 + F9 + F10 + W.1B). F7 tour movido a W.X (ADR-025). Auditoria visual login con usuario: 8 bugs corregidos (BUG-306 a BUG-313) + 10 pendientes documentados. **W.0 Spatial Shell + W.2 Home cinematic IMPLEMENTADOS** 2026-05-04 (CanvasCard 16/9 estandar para todos los modulos, sidebar pill con bell+perfil integrado, EventThemeProvider con accent dinamico Noir+Lux, EventPoster con keyvisual del cliente via `home_card_image_url`, LiveState feed scrolleable conectado a `/api/v1/events/{id}/happening-now`). Despues W.3 Agenda + W.4 Streaming + W.5 Speakers. Deploy staging DO sao1 |
...
_EventOS Roadmap v5.9 — Kasproduction_
_4 mayo 2026_
_Cambios v5.8→v5.9 (2026-05-04): **W.0 Spatial Shell + W.2 Home IMPLEMENTADOS** en `eventos-web` (Next 16 + React 19). Componentes shell: `SpatialShell`, `SidebarPill` (con bell + perfil integrado), `CanvasCard` (16/9 ESTANDAR para todos los modulos — formulas: `width: min(1600, calc(100vw-150), calc((100vh-80)*16/9))`), `AmbientBackground`, `Stage`, `EventThemeProvider`, `ProfilePopover`, `ThemeTogglePill`. Home cinematic + mute por estado (PRE/LIVE/ENDED) con feed scrolleable de salas activas. **Backend extension**: nuevos campos `events.primary_color_light` (accent override Lux) + `events.home_card_image_url` + `events.home_card_overlay` (keyvisual webapp 16:9 separado del app movil). Nuevo controller `HappeningNowController` `/api/v1/events/{id}/happening-now`. Comando artisan `happening:simulate` para activar 4 sesiones live ahora (demo). Helper `resolveAssetUrl` en `PublicEventController` para devolver URLs absolutas del Filament FileUpload. Sistema de accent dinamico via `EventThemeProvider` con `getContrastColor()` automatico (texto sobre accent calculado por luminance). Memorias nuevas: `project_webapp_shell_canvas`, `project_webapp_responsive_strategy`, `feedback_use_existing_button`. Docs actualizados: `W.0-spatial-ui.md`, `W.2-home.md`._
_Cambios v5.7→v5.8 (2026-05-02d): ADR-026 — F10 UI/UX foundation (~6.5h) agregada al final de W.1 con 3 tiers. Tier A Foundation (LuminaToast wrapper Sonner con 5 variantes + haptics web, FormField con AnimatePresence error, EmptyState con 4 variantes, Skeleton patterns, useOptimistic hook). Tier B Polish (Button micro-feedback, focus rings dynamic, page transitions AnimatePresence, stagger entrance, AnimatedNumber ticker, smooth scroll, theme cross-fade, reduced motion tier, gradient breathing pre-event hero, scroll trigger IntersectionObserver, swipe haptics mobile). Tier C Premium (ErrorBoundary con Sentry report, custom 404/500/offline pages, keyboard shortcuts globales, CopyButton, connection status pill latency check, save indicators, SubmittingButton, cross-tab sync logout, page exit guard form dirty). W.1 estimado 9h → 15.5h. Total bloqueante webapp 135.5h → 141h_
_Cambios v5.6→v5.7 (2026-05-02d): ADR-025 supersede ADR-019 — WelcomeShowcase movido de W.1 F7 a nueva W.X tardia (post W.3+W.4+W.5+W.7+W.8+W.9). Razon: showcase muestra 6 modulos (Speakers/Agenda/Streaming/Connect/Gamification/Sponsors) — construirlo en W.1 con placeholders genera codigo de descarte. Reusar componentes reales en miniatura es la decision DaVinci. W.1 cierra con F0+F1+F2+F3+F4+F5+F6+F8+F9+F10 (sin F7). Estimado revisado W.1: 10h → 9h_
