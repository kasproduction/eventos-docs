...
| D.13 | `room:occupancy` dead emit backend | No (nadie lo consume) | `RoomCheckinService:373` emite al room `event:{id}:admin` que NO existe en `Rooms.ts` ni tiene join handler. Verificado contra las 10 superficies (auditoria `AUDITORIA-SOCKETS-SUPERFICIES-2026-07-04.md`): Event Pulse usa `checkin:update` (aforo global), NO este. Probablemente previsto para dashboard de rooms futuro. Decidir: implementar consumidor o remover el emit. |
